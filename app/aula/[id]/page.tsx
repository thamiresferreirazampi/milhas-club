...
export default function AulaPage() {
    ...
    return (
      <main className="min-h-screen bg-brand-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-brand-gray-light text-sm">Carregando aula...</p>
        </div>
      </main>
    )
  }
  ...
  if (!lesson) {
    return (
      <main className="min-h-screen bg-brand-black flex items-center justify-center">
        <p className="text-brand-gray-light">Aula não encontrada.</p>
      </main>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-brand-black text-white">
      ...
      {/* Header */}
      <header className="sticky top-0 z-20 border-b border-brand-border bg-brand-black/95 backdrop-blur-md">
        <div className="max-w-screen-xl mx-auto px-4 h-14 flex items-center justify-between">
          <a href="/dashboard" className="flex items-center gap-2.5 group">
            ...
            <span className="font-extrabold text-white tracking-tight group-hover:text-brand-blue transition-colors">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="hidden sm:flex items-center gap-1.5 text-brand-gray-light hover:text-white text-sm transition-colors"
            >
            ...
            <button
              ...
              className="md:hidden flex items-center gap-2 bg-brand-card border border-brand-border text-brand-gray-light text-sm px-3 py-1.5 rounded-lg transition-all hover:border-brand-blue"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="max-w-screen-xl mx-auto flex">
        ...
        <div className="flex-1 min-w-0 px-4 py-6 md:py-8 md:pr-8">
          ...
          {/* Player */}
          <div className="relative rounded-2xl overflow-hidden bg-black aspect-video mb-6 ring-1 ring-brand-border shadow-2xl shadow-black">
          ...
          <div className="flex items-start justify-between gap-4 mb-5">
            <div className="min-w-0">
              <p className="text-brand-blue text-xs font-semibold uppercase tracking-widest mb-1.5">
                Aula {currentIndex + 1} de {siblings.length}
              </p>
              <h1 className="text-xl md:text-2xl font-bold text-white leading-snug">{lesson.title}</h1>
              <p className="text-brand-gray-light text-sm mt-1.5 flex items-center gap-1.5">
            ...
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-50 ${
                completed
                  ? 'bg-brand-success/10 text-brand-success border border-brand-success/25 hover:bg-brand-success/15'
          ...
          <div className="grid grid-cols-2 gap-3 border-t border-brand-border pt-5">
            {prevLesson ? (
              <a
                ...
                className="group flex flex-col gap-1.5 bg-brand-card hover:bg-brand-card-elevated border border-brand-border hover:border-brand-blue rounded-xl p-4 transition-all"
              >
                <span className="flex items-center gap-1 text-brand-gray-light text-xs">
            ...
            {nextLesson ? (
              <a
                ...
                className="group flex flex-col gap-1.5 bg-brand-card hover:bg-brand-card-elevated border border-brand-border hover:border-brand-blue rounded-xl p-4 transition-all text-right"
              >
                <span className="flex items-center gap-1 justify-end text-brand-gray-light text-xs">
                  Próxima
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Sidebar desktop */}
        <aside className="hidden md:block w-72 flex-shrink-0 border-l border-brand-border">
          <div className="sticky top-14 max-h-[calc(100vh-3.5rem)] overflow-y-auto">
            <div className="p-4 border-b border-brand-border">
              <h2 className="text-sm font-semibold text-white">Aulas do módulo</h2>
              <p className="text-xs text-brand-gray-light mt-0.5">{siblings.length} aulas</p>
            </div>
            <div className="p-2">
              {siblings.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${
                      isActive ? 'bg-brand-blue text-white' : 'bg-brand-border text-brand-gray-light'
                    }`}>
                      {i + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-brand-gray text-xs mt-0.5">{s.duration_minutes} min</p>
                    </div>
                  </a>
                )
              })}
        ...
        <div className="fixed inset-0 z-30 bg-black/70 md:hidden" onClick={() => setSidebarOpen(false)}>
          ...
          >
            <div className="p-4 border-b border-brand-border flex items-center justify-between">
              ...
              <button onClick={() => setSidebarOpen(false)} className="text-brand-gray-light hover:text-white transition-colors">
                ...
                return (
                  <a
                    ...
                    className={`flex items-start gap-3 px-3 py-3 rounded-xl mb-0.5 transition-all border ${
                      isActive
                        ? 'bg-brand-blue/10 border-brand-blue/30'
                        : 'border-transparent hover:bg-brand-card-elevated hover:border-brand-border'
                    }`}
                  >
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${
                      isActive ? 'bg-brand-blue text-white' : 'bg-brand-border text-brand-gray-light'
